...
class MiniImagenet(Dataset):
    ...
    def __init__(self, root, mode, batch_size, n_way, k_shot, k_query, resize, task_id, split=1, startidx=0):
        """
        ...
        if mode == 'train':
            ...
            if not os.path.isfile(os.path.join(root, 'test_support_' + str(n_way) + 'way_' + str(k_shot) + 'shot' + '.json')):
                self.create_batch(self.batch_size)
                with open(os.path.join(root, 'test_support_' + str(n_way) + 'way_' + str(k_shot) + 'shot' + '.json'), 'w+') as f_support:
                    json.dump(self.support_x_batch, f_support)
                with open(os.path.join(root, 'test_query_' + str(n_way) + 'way_' + str(k_shot) + 'shot' + '.json'), 'w+') as f_query:
                    json.dump(self.query_x_batch, f_query)

            else:
                self.support_x_batch = json.load(open(os.path.join(root, 'test_support_' + str(n_way) + 'way_' + str(k_shot) + 'shot' + '.json')))
                self.query_x_batch = json.load(open(os.path.join(root, 'test_query_' + str(n_way) + 'way_' + str(k_shot) + 'shot' + '.json')))


            self.support_x_batch = [self.support_x_batch[task_id]]
            self.query_x_batch = [self.query_x_batch[task_id]]
